...
export default function QuotationGenerator() {
  ...
  const handleGenerateAndSync = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      const searchUbqn = (isEditMode && oldUbqn) ? getStandardizedUBQN(oldUbqn) : standardizedUBQN;
      const { data: existingWorks, error: findError } = await (supabase.from('works' as any)
        .select('id')
        .eq('ubqn', searchUbqn) as any);
